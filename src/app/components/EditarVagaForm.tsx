'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import toast from 'react-hot-toast'
import { Briefcase, Save, Loader2 } from 'lucide-react'

function FormField({ label, children, required = false }: { label: string; children: React.ReactNode; required?: boolean }) {
  return (
    <div>
      <label className="block text-sm font-medium text-gray-700">
        {label}
        {required && <span className="text-red-500 ml-1">*</span>}
      </label>
      <div className="mt-1">{children}</div>
    </div>
  )
}

interface EditarVagaFormProps {
  vaga: { idVaga: number; titulo: string; descricao: string | null }
  onSuccess: () => void
}

export default function EditarVagaForm({ vaga, onSuccess }: EditarVagaFormProps) {
  const [titulo, setTitulo] = useState(vaga.titulo)
  const [descricao, setDescricao] = useState(vaga.descricao || '')
  const [loading, setLoading] = useState(false)
  const router = useRouter()

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!titulo.trim()) {
      toast.error('O título da vaga é obrigatório.')
      return
    }
    setLoading(true)
    try {
      const res = await fetch(`/api/vagas/${vaga.idVaga}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ titulo, descricao }),
      })
      if (res.ok) {
        toast.success('Vaga atualizada com sucesso!')
        onSuccess()
        router.refresh()
      } else {
        const data = await res.json().catch(() => null)
        toast.error(data?.error || 'Não foi possível atualizar a vaga.')
      }
    } catch (_error) {
      console.error(_error)
      toast.error('Ocorreu um erro de comunicação com o servidor.')
    } finally {
      setLoading(false)
    }
  }

  const inputClasses =
    'block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3'

  return (
    <div className="w-full">
      <div className="mb-8 flex items-center space-x-3">
        <Briefcase className="h-8 w-8 text-indigo-600" />
        <h1 className="text-3xl font-bold text-gray-800">Editar Vaga</h1>
      </div>
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
          <FormField label="Título da Vaga" required>
            <input
              type="text"
              value={titulo}
              onChange={(e) => setTitulo(e.target.value)}
              required
              className={inputClasses}
            />
          </FormField>
          <FormField label="Descrição da Vaga">
            <textarea
              value={descricao}
              onChange={(e) => setDescricao(e.target.value)}
              rows={8}
              className={inputClasses}
            />
          </FormField>
        </div>
        <div className="mt-8 flex justify-end border-t border-gray-200 pt-8">
          <button
            type="submit"
            disabled={loading}
            className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {loading ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" /> Atualizando...
              </>
            ) : (
              <>
                <Save className="h-4 w-4" /> Salvar Alterações
              </>
            )}
          </button>
        </div>
      </form>
    </div>
  )
}